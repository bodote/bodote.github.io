...
./localserve-only-published.sh # bundle exec jekyll serve  -> http://localhost:4000
```

The `localserve-only-published.sh` / `serve-unpublished.sh` scripts already pin Ruby 3.3 and a UTF-8
locale, so day to day you can just run them.

To also preview unpublished drafts (posts with `published: false`):

```bash
./serve-unpublished.sh # serve on 0.0.0.0, --watch, --unpublished
```
